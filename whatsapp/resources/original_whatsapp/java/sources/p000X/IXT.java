package p000X;

/* loaded from: classes8.dex */
public abstract class IXT {
    public static final byte[] A00(final byte[] x, final byte[] y) {
        int length = x.length;
        if (length == y.length) {
            return A01(x, y, 0, 0, length);
        }
        throw AbstractC34801aa.A0y("The lengths of x and y should match.");
    }

    public static final byte[] A01(final byte[] x, byte[] offsetX, final int y, int offsetY, int len) {
        if (x.length - len < y || offsetX.length - len < offsetY) {
            throw AbstractC34801aa.A0y("That combination of buffers, offsets and length to xor result in out-of-bond accesses.");
        }
        byte[] bArr = new byte[len];
        for (int i = 0; i < len; i++) {
            AbstractC37200Ghz.A1N(offsetX, bArr, i + offsetY, x[i + y], i);
        }
        return bArr;
    }

    public static byte[] A02(byte[]... chunks) {
        int i = 0;
        for (byte[] bArr : chunks) {
            if (i > Integer.MAX_VALUE - bArr.length) {
                throw AbstractC37199Ghy.A0k("exceeded size limit");
            }
            i += bArr.length;
        }
        byte[] bArr2 = new byte[i];
        int i2 = 0;
        for (byte[] bArr3 : chunks) {
            System.arraycopy(bArr3, 0, bArr2, i2, bArr3.length);
            i2 += bArr3.length;
        }
        return bArr2;
    }
}
