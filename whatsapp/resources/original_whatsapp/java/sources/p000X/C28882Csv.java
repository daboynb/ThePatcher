package p000X;

import com.whatsapp.bloks.wabloks.ui.WaBloksActivity;

/* renamed from: X.Csv, reason: case insensitive filesystem */
/* loaded from: classes6.dex */
public final class C28882Csv implements DSH {
    public final C00V A00 = AbstractC34841ae.A0i();

    @Override // p000X.DSH
    public AbstractC28941Cts AGT(WaBloksActivity waBloksActivity) {
        C00C.A0A(waBloksActivity, 0);
        return new BJE(this.A00, waBloksActivity);
    }

    @Override // p000X.DSH
    public /* bridge */ /* synthetic */ AbstractC28976CuR AGX(WaBloksActivity waBloksActivity) {
        C00C.A0A(waBloksActivity, 1);
        C00V c00v = this.A00;
        C00C.A0A(c00v, 0);
        return new BJD(waBloksActivity, c00v);
    }
}
