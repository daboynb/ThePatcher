package p000X;

import com.facebook.wearable.mediastream.model.SUPToggleState;

/* renamed from: X.8PT, reason: invalid class name */
/* loaded from: classes5.dex */
public final class C8PT extends SUPToggleState {
    public final C188588Nh A00;

    @Override // com.facebook.wearable.mediastream.model.SUPToggleState
    public C9IF getCurrentStatusIndicatorState() {
        return this.A00.A00;
    }

    @Override // com.facebook.wearable.mediastream.model.SUPToggleState
    public SUPToggleState getUpdatedStatusIndicatorAttributes(Boolean bool, C9IF c9if, Boolean bool2, Boolean bool3, Boolean bool4) {
        return new C8PT(this.A00.A01(c9if, bool, bool2, bool3, bool4));
    }

    public C8PT(C188588Nh c188588Nh) {
        this.A00 = c188588Nh;
    }

    public String toString() {
        StringBuilder A04 = AnonymousClass000.A04();
        A04.append("Disconnected(statusIndicatorAttributes=");
        return AbstractC34911al.A0b(this.A00, A04);
    }

    public C8PT() {
        this(new C188588Nh(C8PX.A00, true, false, false, false));
    }
}
