package p000X;

import com.whatsapp.infra.logging.Log;

/* renamed from: X.8dr, reason: invalid class name and case insensitive filesystem */
/* loaded from: classes5.dex */
public final class C193298dr extends AbstractC219009mv {
    public final C0WX A00;
    public final C0X5 A01;
    public final C09570Xb A02;
    public final C0IV A03;
    public final C07T A04;
    public final C218279lI A05;

    public static final void A00(C193298dr c193298dr, C212059a8 c212059a8) {
        InterfaceC21440tC interfaceC21440tC;
        C0IV c0iv = c193298dr.A03;
        AbstractC05520Fq abstractC05520Fq = c212059a8.A01;
        if (c0iv.A0D(abstractC05520Fq) != null) {
            Log.m223i("delete-chat-handler/deleteChat deleteMessagesForRange");
            C218279lI c218279lI = c193298dr.A05;
            boolean z = ((C193078dO) c212059a8.A00).A00;
            C219969op c219969op = c212059a8.A03;
            int A00 = C219969op.A00(((C09570Xb) C05V.A02(c218279lI.A07)).A05(abstractC05520Fq, true), c219969op);
            if (A00 != 2 && A00 != 1) {
                c218279lI.A02(abstractC05520Fq, c219969op, z, true);
                return;
            }
            ((C66972uD) ((C15520jI) C05V.A02(c218279lI.A0B)).A04.get()).A03(abstractC05520Fq, z, false, false);
            C16630l5 A002 = ((C16620l4) C05V.A02(c218279lI.A01)).A00();
            C00C.A06(A002);
            if (!A002.A01) {
                interfaceC21440tC = A002.A00;
                if (interfaceC21440tC == null || interfaceC21440tC.isFinishing() || !abstractC05520Fq.equals(interfaceC21440tC.getChatJid())) {
                    return;
                }
            } else if (!abstractC05520Fq.equals(A002.A00().getChatJid())) {
                return;
            } else {
                interfaceC21440tC = A002.A00();
            }
            interfaceC21440tC.AyP();
        }
    }

    public C193298dr() {
        super(AbstractC34901ak.A0P());
        this.A02 = (C09570Xb) C00H.A02(3623);
        this.A01 = (C0X5) C87U.A0v();
        this.A05 = (C218279lI) C00H.A02(3457);
        this.A00 = (C0WX) C87T.A0w();
        this.A03 = AbstractC34851af.A0T();
        this.A04 = AbstractC34851af.A0U();
    }
}
