package p000X;

import android.net.http.X509TrustManagerExtensions;
import android.util.Base64InputStream;
import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.security.KeyStore;
import java.security.KeyStoreException;
import java.security.NoSuchAlgorithmException;
import java.security.cert.CertificateException;
import java.security.cert.X509Certificate;
import javax.net.ssl.TrustManager;
import javax.net.ssl.TrustManagerFactory;
import javax.net.ssl.X509TrustManager;
import redex.C$StoreFenceHelper;

/* loaded from: classes12.dex */
public final class I2A extends AbstractC79247Vxm implements InterfaceC045003i, InterfaceC044803g {
    public final InterfaceC045003i A00;
    public final X509TrustManager A01;
    public final X509TrustManagerExtensions A02;

    public I2A(InterfaceC045003i interfaceC045003i) {
        super.A00 = interfaceC045003i;
        try {
            KeyStore keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
            keyStore.load(new Base64InputStream(new ByteArrayInputStream("".getBytes()), 0), "password".toCharArray());
            TrustManagerFactory trustManagerFactory = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm());
            trustManagerFactory.init(keyStore);
            TrustManager[] trustManagers = trustManagerFactory.getTrustManagers();
            super.A01 = trustManagers;
            C$StoreFenceHelper.DUMMY_VOLATILE = 0;
            this.A00 = interfaceC045003i;
            X509TrustManager x509TrustManager = (X509TrustManager) trustManagers[0];
            this.A01 = x509TrustManager;
            this.A02 = new X509TrustManagerExtensions(x509TrustManager);
        } catch (IOException | KeyStoreException | NoSuchAlgorithmException | CertificateException unused) {
            throw AnonymousClass011.A0J("Failure initialising sandbox TrustManager");
        }
    }

    @Override // p000X.InterfaceC045003i
    public final void AJw(X509Certificate[] x509CertificateArr, String str) {
        try {
            this.A00.AJw(x509CertificateArr, str);
        } catch (CertificateException unused) {
            this.A02.checkServerTrusted(x509CertificateArr, "ECDHE_ECDSA", str);
        }
    }

    @Override // p000X.InterfaceC044803g
    public final void AJx(X509Certificate[] x509CertificateArr, String str, boolean z) {
        InterfaceC045003i interfaceC045003i = this.A00;
        if (!(interfaceC045003i instanceof InterfaceC044803g)) {
            AJw(x509CertificateArr, str);
            return;
        }
        try {
            ((InterfaceC044803g) interfaceC045003i).AJx(x509CertificateArr, str, z);
        } catch (CertificateException unused) {
            this.A02.checkServerTrusted(x509CertificateArr, "ECDHE_ECDSA", str);
        }
    }
}
