package p000X;

import java.util.concurrent.atomic.AtomicLong;

/* renamed from: X.4cG, reason: invalid class name and case insensitive filesystem */
/* loaded from: classes2.dex */
public abstract class AbstractC116324cG {
    public static final AtomicLong A00 = new AtomicLong(0);
    public static final AtomicLong A05 = new AtomicLong(0);
    public static final AtomicLong A04 = new AtomicLong(0);
    public static final AtomicLong A01 = new AtomicLong(0);
    public static final AtomicLong A02 = new AtomicLong(0);
    public static final AtomicLong A03 = new AtomicLong(0);
    public static final AtomicLong A07 = new AtomicLong(0);
    public static final AtomicLong A08 = new AtomicLong(0);
    public static final AtomicLong A06 = new AtomicLong(0);
    public static final AtomicLong A09 = new AtomicLong(0);
    public static final AtomicLong A0D = new AtomicLong(0);
    public static final AtomicLong A0C = new AtomicLong(0);
    public static final AtomicLong A0A = new AtomicLong(0);
    public static final AtomicLong A0B = new AtomicLong(0);
    public static final AtomicLong A0F = new AtomicLong(0);
    public static final AtomicLong A0E = new AtomicLong(0);
}
